...
- What happens when the `.claude` directory does not exist?
- What happens when `session_history.json` is missing or malformed?
- What happens when a skill directory is empty or missing expected files?
- How does the tool handle circular symlinks or permission errors?
- What happens with very large session histories (thousands of sessions)?
...
- **FR-003**: The CLI MUST support filtering by component type using flags: `--plugins`, `--skills`, `--sessions`, `--mcp`.
...
- **FR-011**: The CLI MUST display a user-friendly error message when the `.claude` directory does not exist.
...
- **Source**: Enumeration of component sources (Official, BuiltIn, Local, ThirdParty).
...
- **SC-003**: All component types (plugins, skills, sessions, MCP) are discoverable via command output.
